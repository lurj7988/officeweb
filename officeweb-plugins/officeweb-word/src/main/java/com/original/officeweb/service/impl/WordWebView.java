...
import com.epoint.officeweb.utils.AsposeWordsUtils;
...
@Component
public class WordWebView implements WebView {

    private final WebViewService webViewService;
    private final static Logger logger = LoggerFactory.getLogger(WordWebView.class);

    public WordWebView(WebViewService webViewService) {
    ...
    public String handle(WebDocument document, Model model) throws Exception {
        if (StringUtils.isBlank(document.getOutfilepath())) {
            try {
                ...
                AsposeWordsUtils.word2pdf(document.getFilepath(), document.getOutfilepath());
